...
export async function generatePitch(lead: Lead): Promise<PitchResult> {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      const aiResult = await generateWithClaude(lead, recommended);
      return { ...aiResult, recommendedProducts: recommended };
    } catch {
...
async function generateWithClaude(
...
): Promise<Omit<PitchResult, "recommendedProducts">> {
  ...
  const productContext = products
    .map((p) => `- ${p.name}${p.sku ? ` (SKU: ${p.sku})` : ""} [${p.category}]: ${p.reason}${p.dealerPrice ? ` | Dealer: ₹${p.dealerPrice}${p.moq ? `, MOQ: ${p.moq}` : ""}` : ""}`)
    .join("\n");
...
Recommended products from our catalog (already matched to their business type):
${productContext}

Generate TWO things:
1. A professional EMAIL PITCH — subject line + email body. Reference their specific business type, mention 2–3 of the matched products by name with their key benefit. Close with a free sample kit offer.
2. A WHATSAPP MESSAGE — casual, punchy, under 180 words. Use *bold* for emphasis, 3–4 emojis, end with "Reply YES for a free sample kit!". Address them as ${firstName}.
